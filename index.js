...
app.get('/app', (request, response) => {
    response.send({mns: 'the endpoint is working!'});
})
...
ioServer.on('connection', (socket) => {
    socket.on('directions', changeDir => {
        socket.broadcast.emit('change-directions', changeDir);
    })
...
app.post('/score', (request, response) => {
    ...
    actualScore += score;
    response.send({mns: 'Your actual score is ' + actualScore});
    console.log('Your actual score is ' + actualScore);
